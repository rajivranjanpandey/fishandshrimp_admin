...
import { deleteAdminCoupon } from '@/app/lib/coupon/action';
...
const Coupons: React.FC = () => {
    ...
    useEffect(() => {
        fetch('/api/coupon').then(res => {
            console.log('res::::::', res);
            if (res.status === 200)
                return res.json();
            else
                return [];
        }).then(dt => setCoupons(dt));
    }, []);
    const onCouponAdd = () => {
        router.push('/admin/coupon/create');
    }
    const onCouponView = (couponId: number) => {
        router.push(`/admin/coupon/${couponId}`);
    }
    const deleteCoupon = (couponId: number, index: number) => {
        fetch('/api/coupon', { method: 'DELETE', body: JSON.stringify({ id: couponId }) }).then(res => res.json()).then(dt => {
            console.log('message', dt.message === 'Coupon deleted successfully');
            if (dt.message === 'Coupon deleted successfully') {
                                ...
                                <tr key={key}>
                                    ...
                                    <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                        <div className="flex items-center space-x-3.5">
                                            <button className="hover:text-primary" onClick={() => onCouponView(couponItem.id, key)}>
                                                <svg
                                            ...
                                            <button className="hover:text-primary" onClick={() => deleteCoupon(couponItem.id)}>
                                                <svg
                                                    className="fill-current"
                                                    width="18"
                                                    height="18"
